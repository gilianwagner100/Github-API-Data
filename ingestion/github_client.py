...
def make_request(url:str, headers:dict, params:dict = None, max_retries:int = 5) -> dict:
    ...
    for attempt in range(max_retries):
        ...
        if response.status_code == 200:
            ...
            if retry_after:
                logger.warning(f"Rate limited. Waiting for {retry_after}s (retry-after) ...")
                ...
                logger.warning(f"Rate limited & will reset at {time_reset}. Sleeping for {sleep_for}s ...")
                time.sleep(sleep_for)

            else:
                if attempt >= max_retries -1:
                    raise Exception(f"Secondary Rate Limit: max retries {max_retries} exceeded.")
                logger.warning(f"Secondary rate limit: waiting for {wait_time}s (attempt {attempt + 1})")
                time.sleep(wait_time)
                wait_time *= 2 # exponential backoff when secondary rate limit is reached as per API doc

        else:
            logger.error(f"HTTP Response Code '{response.status_code} on {url}'")
